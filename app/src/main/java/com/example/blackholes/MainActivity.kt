...
class MainActivity : AppCompatActivity() ,SensorEventListener,SurfaceHolder.Callback{
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        ...
        SensorManager.getRotationMatrix(inR, SensorManager.AXIS_X, SensorManager.AXIS_Y, outR)
    ...
    private fun goaled() : String{
        val elaspsedTime = System.currentTimeMillis() - startTime
        val secTime = (elaspsedTime / 1000).toInt()
    ...
    override fun surfaceCreated(holder: SurfaceHolder?) {
        val sensorManager = getSystemService(Context.SENSOR_SERVICE) as SensorManager
        val accelermater = sensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER)
        val magField = sensorManager.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD)
        sensorManager.registerListener(this, accelermater, SensorManager.SENSOR_DELAY_NORMAL)
